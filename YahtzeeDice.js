document.addEventListener('DOMContentLoaded', function() {
    ...
    async function stopAnimation() {
        try {
            ...
            let response = await fetch('http://localhost:3010/roll-dice');
            let data = await response.json();

            // Output data
            console.log('Dice Numbers ', data.diceNumber);
            console.log('Total ', data.total);
            console.log('Upper Sum ', data.upperSum);

            document.getElementById("totalUpper").innerHTML = data.upperSum;

            // Apply bonus to top total
            if (data.bonus) {
